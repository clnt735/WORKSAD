...
if (isset($_POST['verify'])) {
    ...
    if (time() > $expires) {
        ...
        // For employers, we DO NOT set user_status_id=1 yet.
        // They remain pending (user_status_id=0) until admin approves their verification.
        // We clear the activation_token to mark email as verified.
        
        $activate = $conn->prepare("UPDATE user SET activation_token = NULL, token_expires_at = NULL WHERE user_id = ?");
        $activate->bind_param("i", $user_id);
        $activate->execute();

        // Also update user_profile to keep them in sync
        $updateProfile = $conn->prepare("UPDATE user_profile SET user_status_id = 0 WHERE user_id = ?");
        ...
        header("Location: login.php?status=email_verified"); 
        exit();
    }
}
?>
